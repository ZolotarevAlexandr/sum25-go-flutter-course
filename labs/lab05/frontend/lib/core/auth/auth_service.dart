...
class AuthService {
  ...
  Future<AuthResult> login(String email, String password) async {
    ...
    final safeEmail = FormValidator.sanitizeText(email);

    try {
      final user = await _userRepository.findByEmail(safeEmail);
      if (user == null) return AuthResult.invalidCredentials;

      final ok = await _userRepository.verifyPassword(safeEmail, password);
      if (!ok) return AuthResult.invalidCredentials;
  ...
  bool isSessionValid() {
    ...
    final started = _currentState.loginTime;
    if (started == null) return false;

    final hours = DateTime.now().difference(started).inHours;
    return hours < 24;
  }
